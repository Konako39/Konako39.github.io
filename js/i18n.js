/* ============================================================
 * 多语言 zh / en / ja
 * - zh 的项目内容直接用 projects.js 里的原文
 * - en / ja 在 projects 里覆写 title / type / short / desc / mediaLabels
 * - 首次访问默认日文，之后记住用户选择
 * ============================================================ */

const I18N = {
  /* ================= 中文 ================= */
  zh: {
    ui: {
      htmlTitle: "KONAKO ～被夺走的公主～",
      startTag: "KONAKO GAMES 出品",
      storyHead: "— 序章 —",
      story1: "平静的颜料王国里，<b>公主被魔王抓走了！</b>",
      story2: "传说魔王城的大门需要 <b class='gold-txt'>{n} 把钥匙 🔑</b>，<br>而钥匙就藏在 {n} 份 <b>BOSS 级任务档案</b>的最深处。",
      story3: "读完档案即可拔出钥匙。收集金币、解锁成就，<br>顺便……认识一下这个世界的作者。",
      startHint: "（背景是实时流体模拟。手机上只在你触摸时才泼动，平时不耗电）",
      navStats: "角色", navMain: "主线", navSide: "支线", navGate: "魔王城",
      heroKicker: "STAGE 1 · 颜料王国 · 出发的村庄",
      heroLine: "～ 被夺走的<em>公主</em> ～",
      heroSub: "一位兴趣创作者 以及这个世界的勇者",
      objTag: "当前目标",
      objText: "集齐 <b>{n} 把钥匙 🔑</b> 打开魔王城 · 完成 BOSS 级任务档案即可获得",
      scrollCue: "向下探索地图",
      secStatus: "勇者面板",
      skill1: "游戏开发", skill2: "AI / LLM 应用", skill3: "C++", skill4: "C#",
      statP1: "一位开发者，喜欢从头开始自己动手，把脑海里的想法全部变成现实。",
      statP2: "相信「玩」是最好的了解方式，所以，救出公主吧！",
      secMain: "主线任务",
      secMainSub: "{n} 份 BOSS 级任务档案，每读完一份，就能拔出一把魔王城的钥匙 🔑",
      secSide: "支线任务",
      secSideSub: "不影响主线，但金币和成就都藏在这里——完成支线也有丰厚奖励。",
      secGate: "魔王城",
      gateDesc: "巨大的城门紧闭着。门上有三个钥匙孔，隐约能听到公主的呼救声……",
      gateDescReady: "三把钥匙同时亮起，城门发出低沉的轰鸣——公主就在里面！",
      gateLocked: "⚔ 打开城门（{n} / {m}）",
      gateOpen: "⚔ 打开城门，救出公主！",
      gateAgain: "⚔ 再次进入魔王城",
      secContact: "组队邀请",
      contactLine: "想一起做点什么？发出你的组队邀请：",
      contactMail: "✉ 邮件联络",
      contactFoot: "© 2026 KONAKO · 背景由实时流体模拟驱动，每个人看到的颜料都不一样。",
      resetLink: "🗑 重置存档，开始新的冒险",
      qlToggle: "📜 任务日志",
      qlMain: "⚔ 主线 · 拯救公主", qlSide: "🗺 支线 · 世界探索",
      qlKeys: "钥匙：", qlCoins: "💰 金币：", qlAch: "🏆 成就：",
      qlAchAll: "🏆 成就大全", achLocked: "？？？未解锁",
      backToMap: "返回地图", nextFile: "下一份档案", zoomHint: "点击图片放大查看",
      bossTag: "★ BOSS 档案", sideTag: "SIDE QUEST",
      keyTagHas: "🔑 深处藏着一把钥匙", keyTagDone: "🔑 钥匙已到手",
      openFile: "▶ 打开任务档案", reopenFile: "✔ 重读档案",
      viewIntel: "查看情报 →", viewIntelDone: "✔ 已完成 · 再看看",
      steamBtn: "🎮 Steam 商店页（加个愿望单？）",
      paperBtn: "📄 arXiv 论文",
      techLabel: "🔧 使用技术",
      vibeLabel: "VibeCoding",
      slotText: "新任务筹备中…",
      slotHint: "在 js/projects.js 里加一段配置，这里就会出现新项目",
      modalFile: "任务档案",
      completeMain: "🔑 读毕档案 · 拔出钥匙（+150 金币）",
      completeSide: "✅ 完成支线任务（+80 金币）",
      doneMark: "✔ 该任务已完成", doneMarkKey: " · 钥匙已到手 🔑",
      doneStamp: "任务完成",
      gifLoad: "点击加载动图", gifWait: "（{size}，请耐心等待）",
      shopTitle: "🛒 颜料王国杂货铺",
      shopSub: "用冒险攒下的金币，换点好玩的。",
      buy: "购买", owned: "已拥有", on: "开启中 · 点击关闭", off: "已关闭 · 点击开启",
      use: "再来一次", read: "阅读",
      noCoinsT: "金币不足", noCoinsD: "去完成任务或捡金币吧！",
      boughtT: "购买成功", boughtD: "金币 -{p}",
      shopFireworks: "颜料烟花", shopFireworksD: "在画布上炸开一大波颜料烟花（可重复购买）。",
      shopGoldrain: "黄金雨", shopGoldrainD: "让金币从天而降 15 秒（可重复购买）。",
      shopBrush: "超级泼溅笔刷", shopBrushD: "鼠标颜料泼溅范围 ×2.6，永久解锁，可随时开关。",
      shopRainbow: "彩虹颜料", shopRainbowD: "整个画布进入彩虹循环模式，永久解锁，可随时开关。",
      shopLetter: "公主的感谢信", shopLetterD: "一封来自公主的亲笔信，据说写了些真心话。",
      letterTitle: "💌 公主的信",
      letterBody: "「亲爱的冒险者：<br><br>谢谢你一路收集钥匙来救我。<br>其实魔王并不坏，他只是想让更多人停下来，<br>好好看看 KONAKO 做的这些东西。<br><br>如果你也喜欢这个世界——<br>去 GitHub 点颗星、去 Steam 加个愿望单，<br>就是对勇者最好的报答了。<br><br>—— 公主 敬上」",
      close: "关闭",
      quips: ["别戳了，会掉血的！", "金币不够？去支线看看。", "公主还在等你哦。", "背景其实是个流体模拟，我写的。", "听说连续戳五次有惊喜。"],
      ach: {
        begin: ["冒险开始", "为了公主！也为了……了解一下作者。"],
        main: ["发现主线任务", "BOSS 级档案进入视野。"],
        side: ["支线猎人", "你发现了隐藏在地图各处的支线任务。"],
        gate: ["兵临城下", "你抵达了魔王城的大门。"],
        intel: ["情报收集", "你打开了第一份任务档案。"],
        quest1: ["初露锋芒", "完成了第一个任务，冒险才刚刚开始。"],
        allquests: ["全清玩家", "主线支线一个不落，你是真正的完美主义者。"],
        keymaster: ["开锁大师", "集齐了魔王城的三把钥匙。"],
        painter: ["颜料大师", "在画布上泼了 10 次颜料，你已经是半个画家了。"],
        rich: ["小富翁", "存款突破 500 金币，可以在王国买栋小房子了。"],
        coinall: ["金币收藏家", "地图上散落的金币被你捡得一枚不剩。"],
        patience: ["实机鉴赏家", "点开了实机演示视频，看得很认真。"],
        explore: ["全图探索", "你走完了这张地图的每一个角落。"],
        curious: ["未来的冒险", "连还没发布的任务都要点一下，好奇心满分。"],
        poke: ["老朋友", "戳了勇者五次，你们已经很熟了。"],
        konami: ["隐藏代码", "↑↑↓↓←→←→BA——你果然是老玩家。"],
        hero: ["传说勇者", "打开魔王城，救出了公主。"],
        steam: ["愿望单骑士", "你点开了 Steam 商店页——这是对开发者最大的鼓励。"],
        shopper: ["剁手党", "在杂货铺完成了第一笔消费。"],
      },
      achToast: "成就解锁 · ",
      keyGotT: "拔出了钥匙 · {t}", keyGotD: "获得魔王城钥匙（{n}/{m}）+150 金币",
      sideDoneT: "任务完成 · {t}", sideDoneD: "支线奖励 +80 金币",
      coinT: "获得 {n} 金币", coinD: "钱包越来越鼓了。",
      endPlace: "魔王城 · 最深处",
      endBoss1: "魔王：「什么？！你居然读完了所有的 BOSS 级档案……」",
      endSideNone: "魔王：「哼，只做主线的效率玩家吗……也罢！」",
      endSide_project2: "魔王：「魔女猫的塔你也爬了？！那游戏连我都通不了……」",
      endSide_project3: "魔王：「你还和 Lux 聊过天？她是不是把我的弱点都说出去了！」",
      endSide_art: "魔王：「画廊的每一幅画都看过了？连那幅火焰研究都……」",
      endSide_blender: "魔王：「连 3D 手办的裙褶都数过了？你也太认真了吧！」",
      endBossLose: "魔王：「这份热情……我、我认输了！」",
      endRescued: "公主获救了！",
      endTreasure: "公主：「其实我早就想说——真正的宝物，是这一路上你认识的 KONAKO 呀。」",
      endFin: "— FIN · 感谢游玩 —",
      endAgain: "🔁 二周目（重置存档）", endFree: "🗺 继续自由探索",
      endCoins: "💰 金币", endAch: "🏆 成就", endQuests: "📜 任务",
    },
  },

  /* ================= English ================= */
  en: {
    ui: {
      htmlTitle: "KONAKO ~ The Stolen Princess ~",
      startTag: "A KONAKO GAMES PRODUCTION",
      storyHead: "— PROLOGUE —",
      story1: "In the peaceful Kingdom of Paint, <b>the Princess has been kidnapped by the Demon King!</b>",
      story2: "Legend says the castle gate needs <b class='gold-txt'>{n} keys 🔑</b>,<br>hidden deep inside {n} <b>BOSS-class quest files</b>.",
      story3: "Read a file to pull out its key. Collect coins, unlock achievements —<br>and get to know the author of this world along the way.",
      startHint: "(The background is a live fluid simulation. On phones it only stirs when you touch it.)",
      navStats: "Status", navMain: "Main", navSide: "Side", navGate: "Castle",
      heroKicker: "STAGE 1 · KINGDOM OF PAINT · STARTING VILLAGE",
      heroLine: "~ The Stolen <em>Princess</em> ~",
      heroSub: "A hobbyist creator — and the hero of this world",
      objTag: "OBJECTIVE",
      objText: "Collect <b>{n} keys 🔑</b> to open the Demon Castle · finish BOSS-class quest files to earn them",
      scrollCue: "Scroll to explore the map",
      secStatus: "Hero Status",
      skill1: "Game Dev", skill2: "AI / LLM", skill3: "C++", skill4: "C#",
      statP1: "A developer who loves building from scratch, turning every idea in their head into reality.",
      statP2: "I believe playing is the best way to know someone — so go save the princess!",
      secMain: "Main Quests",
      secMainSub: "{n} BOSS-class quest files. Finish reading one, pull out one castle key 🔑",
      secSide: "Side Quests",
      secSideSub: "Optional — but that's where the coins and achievements hide. Side quests pay well too.",
      secGate: "Demon Castle",
      gateDesc: "The great gate is sealed. {n} keyholes... and a faint cry for help from inside.",
      gateDescReady: "Every key blazes at once and the gate rumbles — the Princess is right inside!",
      gateLocked: "⚔ Open the gate ({n} / {m})",
      gateOpen: "⚔ Open the gate and save the Princess!",
      gateAgain: "⚔ Enter the castle again",
      secContact: "Party Invite",
      contactLine: "Want to build something together? Send a party invite:",
      contactMail: "✉ Email me",
      contactFoot: "© 2026 KONAKO · The background is a realtime fluid simulation — everyone sees different paint.",
      resetLink: "🗑 Reset save & start a new adventure",
      qlToggle: "📜 Quest Log",
      qlMain: "⚔ Main · Save the Princess", qlSide: "🗺 Side · World Exploration",
      qlKeys: "Keys: ", qlCoins: "💰 Coins: ", qlAch: "🏆 Achievements: ",
      qlAchAll: "🏆 Achievement List", achLocked: "??? Locked",
      backToMap: "Back to map", nextFile: "Next file", zoomHint: "Click image to zoom",
      bossTag: "★ BOSS FILE", sideTag: "SIDE QUEST",
      keyTagHas: "🔑 A key lies deep inside", keyTagDone: "🔑 Key obtained",
      openFile: "▶ Open quest file", reopenFile: "✔ Re-read file",
      viewIntel: "View intel →", viewIntelDone: "✔ Done · view again",
      steamBtn: "🎮 Steam page (wishlist it?)",
      paperBtn: "📄 arXiv paper",
      techLabel: "🔧 Tech stack",
      vibeLabel: "VibeCoding",
      slotText: "New quest in preparation…",
      slotHint: "Add one config block in js/projects.js and a new project appears here",
      modalFile: "QUEST FILE",
      completeMain: "🔑 File read · Pull out the key (+150 coins)",
      completeSide: "✅ Complete side quest (+80 coins)",
      doneMark: "✔ Quest completed", doneMarkKey: " · Key obtained 🔑",
      doneStamp: "CLEARED",
      gifLoad: "Click to load GIF", gifWait: "({size}, please be patient)",
      shopTitle: "🛒 Kingdom General Store",
      shopSub: "Trade your hard-earned coins for fun stuff.",
      buy: "Buy", owned: "Owned", on: "ON · click to disable", off: "OFF · click to enable",
      use: "Use again", read: "Read",
      noCoinsT: "Not enough coins", noCoinsD: "Finish quests or pick up coins!",
      boughtT: "Purchased", boughtD: "Coins -{p}",
      shopFireworks: "Paint Fireworks", shopFireworksD: "Blast a huge wave of paint across the canvas (repeatable).",
      shopGoldrain: "Golden Rain", shopGoldrainD: "Make coins fall from the sky for 15 seconds (repeatable).",
      shopBrush: "Mega Splat Brush", shopBrushD: "Mouse paint splats ×2.6 size. Permanent unlock, toggle anytime.",
      shopRainbow: "Rainbow Paint", shopRainbowD: "The whole canvas cycles through rainbow hues. Permanent unlock, toggle anytime.",
      shopLetter: "Princess's Letter", shopLetterD: "A handwritten letter from the Princess. Heartfelt, they say.",
      letterTitle: "💌 A Letter from the Princess",
      letterBody: "\"Dear adventurer,<br><br>Thank you for collecting the keys to save me.<br>The Demon King isn't evil, really — he just wanted more people<br>to stop and look at what KONAKO has made.<br><br>If you liked this world,<br>a star on GitHub or a wishlist on Steam<br>is the finest reward a hero could give.<br><br>— The Princess\"",
      close: "Close",
      quips: ["Stop poking, I'll lose HP!", "Low on coins? Try the side quests.", "The Princess is still waiting.", "The background is a fluid sim. I wrote it.", "They say poking five times does something."],
      ach: {
        begin: ["Adventure Begins", "For the Princess! And... to meet the author."],
        main: ["Main Quest Found", "Three BOSS-class files in sight."],
        side: ["Side Quest Hunter", "You found the side quests scattered across the map."],
        gate: ["At the Gates", "You reached the Demon Castle."],
        intel: ["Intel Gathered", "You opened your first quest file."],
        quest1: ["First Blood", "First quest done — the adventure has just begun."],
        allquests: ["Completionist", "Every main and side quest cleared. A true perfectionist."],
        keymaster: ["Keymaster", "All castle keys collected."],
        painter: ["Paint Master", "Ten splats on the canvas — half a painter already."],
        rich: ["Moneybags", "Over 500 coins. You could buy a cottage in the kingdom."],
        coinall: ["Coin Collector", "Not a single coin left on the map."],
        patience: ["Gameplay Connoisseur", "You pressed play on a gameplay video and watched closely."],
        explore: ["Full Map Explored", "You walked every corner of this map."],
        curious: ["Future Adventure", "You even clicked the unreleased quest. Full marks for curiosity."],
        poke: ["Old Friends", "Poked the hero five times. You're close now."],
        konami: ["Hidden Code", "↑↑↓↓←→←→BA — a veteran gamer indeed."],
        hero: ["Legendary Hero", "Opened the castle and saved the Princess."],
        steam: ["Wishlist Knight", "You opened the Steam page — the best support a dev can get."],
        shopper: ["Big Spender", "Made your first purchase at the store."],
      },
      achToast: "Achievement · ",
      keyGotT: "Key obtained · {t}", keyGotD: "Castle key ({n}/{m}) +150 coins",
      sideDoneT: "Quest complete · {t}", sideDoneD: "Side reward +80 coins",
      coinT: "Got {n} coins", coinD: "The wallet grows heavier.",
      endPlace: "DEMON CASTLE · INNERMOST HALL",
      endBoss1: "Demon King: \"What?! You actually read every BOSS-class file...\"",
      endSideNone: "Demon King: \"Hmph, a main-quest-only speedrunner... so be it!\"",
      endSide_project2: "Demon King: \"You even climbed the Witch Cat tower?! I can't even beat that game...\"",
      endSide_project3: "Demon King: \"You talked to Lux too? Did she tell you all my weaknesses?!\"",
      endSide_art: "Demon King: \"You saw every painting in the gallery? Even the flame study...\"",
      endSide_blender: "Demon King: \"You counted the frills on the 3D figure's skirt? That's dedication!\"",
      endBossLose: "Demon King: \"Such passion... I... I surrender!\"",
      endRescued: "The Princess is saved!",
      endTreasure: "Princess: \"I always wanted to say — the real treasure is the KONAKO you got to know along the way.\"",
      endFin: "— FIN · THANKS FOR PLAYING —",
      endAgain: "🔁 New Game+ (reset save)", endFree: "🗺 Keep exploring",
      endCoins: "💰 Coins", endAch: "🏆 Achievements", endQuests: "📜 Quests",
    },
    projects: {
      qianhua: {
        title: "Desktop Girl Chika", type: "LLM-driven Desktop Companion Life Sim · ON STEAM",
        tags: ["AI Companion", "Long-term Memory", "Emotion System", "Agent Tool Use", "On Steam"],
        short: "A girl who lives inside your computer: she has emotions, memories and a life of her own — and she can actually act inside the space you grant her.",
        desc: [
          "Chika wakes up in a blank virtual world, and you bring her home to live inside your computer. From that day on you are the first person she has ever known, and the only window between her and the real world. She knows she lives in a machine yet stubbornly wants to become a real human — so she starts watching you, remembering you, and through every moment of joy, shyness, sadness and argument, asking what 'being alive' is still missing.",
          "Under the hood she is one complete agent: chatting, recalling, writing her diary, posting, playing chess and tidying her room are all driven by the same persona — there is no split between a 'chat mode' and an 'agent mode'. Inside the workspace you explicitly grant her she can look things up, organize files and edit text, and she cannot take a single step past that boundary — you always decide how much world she gets to touch.",
        ],
        mediaLabels: ["The girl who moved into your computer", "Emotions that actually flow", "Tmitter · her own social feed", "Sit down for a game of chess", "Diary and long-term memory", "Her nest is a real folder on your desktop"],
        mediaTexts: [
          "Chika isn't a picture pinned to your desktop. She stands there as a 3D VRM character, blinking, looking your way, finding something to do when you go quiet for too long — while insisting 'it's not like I kept the app open because I wanted to see you or anything.'",
          "Happy, angry, shy, sad and calm aren't five labels stuck to a character. Her moods are moved by what's being said, influence each other and settle over time, then surface naturally through her expression, motion, tone and choices — praise makes her happy, being ignored makes her sad, and she only gets shy once she likes you enough.",
          "Chika has her own phone. She posts about her day unprompted, likes and replies to your posts, and slips into DMs for things only the two of you can see. When to pick up the phone, whether to reply, whether to reach out first — all her call. What she was too embarrassed to say out loud may be sitting quietly behind that notification dot.",
          "Chinese chess and gomoku ship with a built-in AI engine whose difficulty tracks your win rate. Chika reads the board, offers advice, gets excited over a beautiful move and talks tough after a loss. She doesn't swap personas to become your opponent — the board is just another afternoon in a life you share, and the result goes into her memory, her feed and her diary.",
          "Every time midnight rolls over she writes the day into her diary — not an action log, not a chat summary, but the day as she saw it. Long-term memory is recalled on demand, and the preferences, promises and shared moments slowly build a history that belongs only to the two of you. The longer it runs, the less any fresh save file could ever reproduce her.",
          "Living room, kitchen, bedroom — furniture, food and toys. Chika's nest is wired straight to a real folder. Move her things in your file explorer and she notices immediately (and holds a grudge). She also works for coins, buys food, adds furniture, tidies up, and slowly falls asleep on the nights you never show up.",
        ],
      },
      mc: {
        title: "Handwritten Minecraft", type: "3D Voxel Sandbox · Built from scratch",
        tags: ["Voxel Terrain", "Chunk Meshing", "Lighting & Day/Night", "Save & Load"],
        short: "A block world written line by line in C++ and modern OpenGL, starting from an empty window — terrain, lighting, water, day/night, digging, placing and saves, all implemented by hand.",
        desc: [
          "No game engine — just an empty window and an OpenGL context. Terrain is generated with Perlin noise, blocks are managed in chunks, and only the visible faces are ever meshed and sent to the GPU. From that foundation, the world slowly grows hills, seas and forests.",
          "Everything built on top is hand-written too: a first-person camera with collision, raycast digging and placing, a hotbar and inventory, translucent water and fog, a day/night sky that shifts with time, plus seed-based world generation with save and load — no ready-made framework underneath.",
        ],
        mediaLabels: ["The whole world from above", "Waterline and shore", "Forest panorama", "Day, night and sky", "First person · dig & place", "Inventory · all the blocks", "Title screen"],
        mediaTexts: [
          "Looking back from high up: hills laid out by Perlin noise, low ground flooded with water, scattered trees, the distance softly closed off by fog. Chunks load around the player, and every hidden face is culled away — only the triangles that actually need drawing remain.",
          "A stretch of shoreline: water is a translucent block, and you can see the sand and terrain beneath it. Beach, grass and water flow into one another, the edges stepping down block by block.",
          "A forest panorama from high ground. Tree positions are decided by a hash of coordinates and seed, so the same seed always grows the exact same world — even trees straddling a chunk seam aren't cut in half.",
          "The same world at night. Sky color and brightness shift smoothly with the time of day — brightest at noon, darkest at midnight, easing through dawn and dusk — and the fog follows the sky.",
          "What it looks like to actually play: the crosshair marks where you dig, left click breaks, right click places, the hotbar shows the block in hand, and number keys or the scroll wheel switch slots.",
          "Open the inventory and there's every block the world uses — grass, dirt, stone, log, planks, brick, glass, sand, snow… click one to hold it and drop it into the hotbar.",
          "The game's title screen. Tiled dirt background, a bitmap-font title and menu buttons — all drawn by the hand-written UI system. New world and load save both start here.",
        ],
      },
      project1: {
        title: "Lost-Heart Samurai", type: "2D Pixel Action Adventure",
        tags: ["Pixel Art", "Action Combat", "Boss Fights", "Level Design"],
        short: "A side-scrolling pixel slasher through a rain-soaked dark forest — full combo, skill, status and boss AI combat systems.",
        desc: [
          "A dark-fairytale side-scrolling pixel action game. You play a samurai who has lost his heart, wandering rain-soaked giant forests and gloomy dungeons, slaying mushroom fiends in search of what he lost.",
          "A complete action system: multi-hit combos, skills, status effects (burn / poison / stun), enemy AI behavior trees, and cinematic boss fights with story dialogue.",
        ],
        mediaLabels: ["Boss Fight · Lost-Heart Samurai", "Dungeon story scene", "Exploration", "Samurai combo demo", "Skill system", "Status effect system", "Enemy AI", "Gameplay flow"],
        mediaTexts: [
          "One of the game's core set-pieces: a boss fight in the rain-soaked forest. HP / MP / stamina bars and the boss health bar form a complete combat HUD, with rain, light shafts and fireflies all driven by particles and lighting. The boss gets its own entrance cutscene.",
          "A story beat by the dungeon campfire: a mysterious traveler points the way. Dialogue box, pixel font and scene lighting are all hand-made — warm firelight breathing against the dark dungeon.",
          "Scene design for exploration: lighting and fog build depth, with switches, hidden passages and secret items rewarding players who wander.",
          "Multi-hit combos in action: light and heavy attacks chain freely, each with its own wind-up, hit feedback and hit-stop tuning — game feel is where most of the polish went.",
          "The full skill loop — cast, cooldown, resource cost — paired with frame-by-frame impact VFX. Skills can cancel combo chains, forming your own move list.",
          "Status effects — burn, poison, stun — stacking and resolving: icons, particles and damage numbers all linked, sharing one state machine for players and enemies alike.",
          "Enemy AI behavior trees: detection, chase, attack and retreat transitions, with distinct aggro ranges and aggression per enemy type.",
          "A complete gameplay slice: exploration into combat into cutscene, showing the game's overall rhythm.",
        ],
      },
      steam_exit: {
        title: "The Exit on Floor 10", type: "3D Paranormal Anomaly Escape · ON STEAM",
        tags: ["Anomaly Spotting", "Horror", "3D", "On Steam"],
        tech: ["Unreal Engine", "Blueprint"],
        short: "The elevator keeps returning you to the 'same' floor — spot up to 31 anomalies and reach the true EXIT.",
        desc: [
          "You've broken into an office building haunted by past murders. The elevator keeps taking you back to a seemingly identical floor — but each one may hide subtle anomalies, with up to 31 differences to spot.",
          "Judge correctly to ascend; guess wrong and you fall back to the start — while dodging the paranormal threats lurking on each floor. Reach the final EXIT and find the true ending! Now on Steam — wishlist it!",
        ],
        mediaLabels: ["A perfectly normal office floor", "Spot the anomaly", "Something's off on every floor", "...it's right there", "Reach the EXIT, or fall back?"],
        mediaTexts: [
          "It all starts in this utterly ordinary office. Memorize every desk, every plant — next time the elevator opens, they might not be the same.",
          "Up to 31 anomalies are hidden in the game: shifted furniture, changed lights, things that simply shouldn't exist. Observation is your survival skill.",
          "Lighting, textures and sound can all be clues — or red herrings. UE's dynamic lighting keeps every floor's 'wrongness' hidden just right.",
          "The red room. Some anomalies aren't content to be 'spotted' — they come looking for you. When that happens: run.",
          "Judge right and ascend; judge wrong and fall back to the start. Survive ten floors to reach the EXIT and the true ending.",
        ],
      },
      steam_civ: {
        title: "Digital Civilization: Evolution", type: "Civilization Sim / Strategy · ON STEAM",
        tags: ["Simulation", "Strategy", "Civilization", "On Steam"],
        short: "Start with a handful of primitives: manage resources, build, negotiate — and evolve into the mightiest empire.",
        desc: [
          "Start from a primitive tribe: manage food, wood and stone, build housing and workshops, grow your population and spark technology — step by step raising a small tribe into an unrivaled empire.",
          "A deep numbers-driven simulation: job assignment, build queues, tech trees, random events and diplomacy — every decision rewrites your civilization's fate. Now on Steam!",
        ],
        mediaLabels: ["Colony construction", "Tech tree · primitive to future", "Resources & jobs", "Event log · your civilization's story", "Dawn of an empire"],
        mediaTexts: [
          "The colony construction panel: build queues and timelines for housing, production and military buildings. Labor is the foundation — with spare housing, people start families and welcome new life.",
          "A layered tech tree spanning the whole history of civilization: every technology genuinely changes production, population and social parameters rather than being a flat unlock. Research direction shapes what your civilization becomes.",
          "Warehouse and workforce systems: production chains for food, wood and stone, and job allocation across hunters, loggers and builders — every adjustment ripples through the economy.",
          "The event log records your civilization's every day: births and deaths, constructions and glories, the traveler you turned away, the tribesman taken by beasts... your civilization's story writes itself.",
          "When the tribe becomes an empire, look back at the log — behind every number is a decision you made. A native web simulation game built entirely with Vue.",
        ],
      },
      project2: {
        title: "Witch Cat Climb", type: "Casual Jumper · Portrait",
        tags: ["Watercolor × Pixel", "Casual", "Magic"],
        short: "A witch-hat black cat leaps up watercolor city towers, casting magic to break its max-height record.",
        desc: [
          "Control a pixel black cat in a witch hat, leaping ever upward between watercolor European buildings — dodging obstacles, casting magic, chasing your best height.",
          "An art experiment mixing watercolor backgrounds with pixel characters: soft city colors against a crisp pixel cat, a contrast both striking and adorable.",
        ],
        mediaLabels: ["Above the clouds", "Gameplay · climbing", "Level design", "Player controls demo", "NPC interaction"],
        mediaTexts: [
          "The higher you fly, the more the scenery changes — breaking through the sea of clouds is the emotional peak of the game.",
          "Watercolor European buildings + a pixel black cat: current and best heights at the top, the magic button at the bottom. Soft versus sharp is the whole art thesis of this game.",
          "Level design sheets: platform rhythm, item placement and the difficulty curve — the higher you go, the tighter the landings and the more magic matters.",
          "Jump feel and spellcasting in action: short tap for a hop, long press for a leap, plus one mid-air magic save.",
          "The NPCs along the way: some bring surprises, some just get in the way — they make every climb a little different.",
        ],
      },
      project3: {
        title: "Lux · AI Companion", type: "AI / Discord Bot",
        tags: ["LLM", "Long-term Memory", "AI Roleplay", "Paper"],
        short: "Lux, an LLM-powered AI character: a Discord companion with long-term memory who can also meet you inside a game.",
        desc: [
          "Lux is an AI character built on large language models. She lives in a Discord server and remembers what every friend has said — greet her weeks later and she still knows you.",
          "The same Lux is also wired into an in-game dialogue UI: the friend you met on Discord can meet you again inside a game world. Covers long-term memory management, persona prompt engineering and a cross-platform dialogue pipeline — with a published arXiv paper.",
        ],
        mediaLabels: ["Discord × in-game dialogue", "Long-term memory demo", "System flowchart", "Lux character art"],
        mediaTexts: [
          "One Lux, two worlds: the in-game dialogue UI on the left, the Discord chat log on the right — one shared memory and personality across platforms.",
          "Long-term memory in practice: greet her a day later and Lux still knows who you are and what you talked about — retrieval and writing are fully transparent to the user.",
          "The system pipeline: message → memory retrieval → persona prompt assembly → LLM inference → dual-platform output. The memory work behind this became an arXiv paper.",
          "Lux's character design: blue hair, a sailor uniform, and just a little shyness — 'Wait, are we... really meeting?'",
        ],
      },
      art: {
        title: "Art Gallery", type: "Illustration / Concept Art",
        tags: ["Oil-paint Style", "Concept Scenes", "Characters"],
        tech: ["Drawing Tablet", "Impasto"],
        short: "From a lake ablaze with autumn leaves to icebergs and castles — a batch of boldly colored digital paintings.",
        desc: [
          "A digital painting collection: a rowboat on a lake drowned in red maple leaves, a castle in fog, polar icebergs, city dusk... I favor bold color blocks and thick brushwork — let the colors tell the story.",
          "Also includes character and fan art — witches, heroes, and some strange but lovable creatures.",
        ],
        mediaLabels: ["Maple Lake", "Castle in Fog", "Iceberg", "The City", "Blade Aura", "Flame Study", "Witch", "Hero", "Loser", "Link · fan art"],
        mediaTexts: [
          "The one where autumn is set on fire: orange-red floods the frame, leaving only the rower's cool blue as a breathing space. Impasto strokes and backlit trunks carry the rhythm.",
          "An atmospheric perspective study in grey: fog pushes the castle away, a few highlights pull the eye back.",
          "A light study in the cold range: ice translucency, sunken sea colors, layers found inside blue.",
          "City silhouettes at dusk color temperature — the moment light leaks between buildings.",
          "A motion study: brush direction and speed lines carrying the energy of a single slash.",
          "Deconstructing fire: from the twist of flame tongues to scattering sparks — a pure element study.",
          "Character design: a witch and her color story.",
          "A hero study — which incidentally became the origin of this website's whole story.",
          "A small-feelings doodle. Felt better after painting it.",
          "Wind of Hyrule: a fan tribute to one of my favorite games ever.",
        ],
      },
      blender: {
        title: "3D Workshop", type: "3D Modeling / Rendering",
        tags: ["Blender", "Figure Render", "Scenes"],
        short: "The Blender room: a Lolita Miku figure, dessert still-lifes, a wooden cabin, and moving VR experiments.",
        desc: [
          "Little worlds built in Blender: a Lolita Miku figure with layer upon layer of cloth frills, dessert still-life renders of cake and cola, cabin scenes and a winding-stream garden.",
          "And experiments in making them move — turntable animations and VR scene tests.",
        ],
        mediaLabels: ["Miku · Lolita figure", "BA fan render", "Cake still-life", "Cola", "Winding Stream", "Cabin scene", "Small scene", "Miku turntable animation", "VR scene experiment"],
        mediaTexts: [
          "The workshop's signature piece: cloth frills modeled layer by layer, tackling the pannier structure and lace-edge detail — the 'three extra layers of frills' obsession started here.",
          "Blue Archive fan render: a likeness study focused on hair segment modeling and toon-shading outline control.",
          "Dessert still-life: subsurface scattering in the cream, highlights on the fruit — material studies that make you hungry.",
          "A refraction ensemble of liquid, glass and ice — a Cycles caustics exercise.",
          "An eastern miniature: winding-stream water material and garden staging.",
          "Natural lighting practice: morning light through a cabin window.",
          "A slice-of-life micro scene — the lived-in feel comes from stacked details.",
          "A 360° turntable: the skirt's layered structure revealed in rotation.",
          "An experiment in making scenes walkable in VR — next step: putting the figures inside.",
        ],
      },
    },
  },

  /* ================= 日本語 ================= */
  ja: {
    ui: {
      htmlTitle: "KONAKO ～奪われた姫～",
      startTag: "KONAKO GAMES プレゼンツ",
      storyHead: "— 序章 —",
      story1: "平和な絵の具王国で、<b>姫が魔王にさらわれた！</b>",
      story2: "魔王城の門を開くには <b class='gold-txt'>{n}本の鍵 🔑</b> が必要。<br>鍵は{n}つの<b>BOSS級クエストファイル</b>の奥深くに眠っている。",
      story3: "ファイルを読み終えれば鍵が手に入る。コインを集め、実績を解除し、<br>ついでに……この世界の作者と仲良くなろう。",
      startHint: "（背景はリアルタイム流体シミュレーション。スマホでは触れたときだけ動きます）",
      navStats: "ステータス", navMain: "メイン", navSide: "サイド", navGate: "魔王城",
      heroKicker: "STAGE 1 · 絵の具王国 · はじまりの村",
      heroLine: "～ 奪われた<em>姫</em> ～",
      heroSub: "趣味クリエイター、そしてこの世界の勇者",
      objTag: "現在の目標",
      objText: "<b>{n}本の鍵 🔑</b> を集めて魔王城を開けよう · BOSS級ファイルを読破すると入手",
      scrollCue: "下へスクロールして探索",
      secStatus: "勇者ステータス",
      skill1: "ゲーム開発", skill2: "AI / LLM", skill3: "C++", skill4: "C#",
      statP1: "一から自分の手で作り上げるのが好きな開発者。頭の中のアイデアを、全部現実にしていきます。",
      statP2: "「遊ぶ」ことが最高の自己紹介だと信じています。さあ、姫を救い出そう！",
      secMain: "メインクエスト",
      secMainSub: "BOSS級クエストファイルは{n}つ。1つ読み終えるごとに、魔王城の鍵を1本入手 🔑",
      secSide: "サイドクエスト",
      secSideSub: "メインには影響しないけど、コインと実績はここに隠れている——報酬もたっぷり。",
      secGate: "魔王城",
      gateDesc: "巨大な城門は固く閉ざされている。{n}つの鍵穴……中からかすかに姫の声が聞こえる。",
      gateDescReady: "すべての鍵が同時に輝き、城門が低く唸る——姫はすぐそこだ！",
      gateLocked: "⚔ 城門を開く（{n} / {m}）",
      gateOpen: "⚔ 城門を開けて姫を救出！",
      gateAgain: "⚔ もう一度魔王城へ",
      secContact: "パーティー募集",
      contactLine: "一緒に何か作りませんか？パーティー申請はこちら：",
      contactMail: "✉ メールで連絡",
      contactFoot: "© 2026 KONAKO · 背景はリアルタイム流体シミュレーション。見える絵の具は人それぞれ。",
      resetLink: "🗑 セーブデータを消して新しい冒険へ",
      qlToggle: "📜 クエストログ",
      qlMain: "⚔ メイン · 姫の救出", qlSide: "🗺 サイド · 世界探索",
      qlKeys: "鍵：", qlCoins: "💰 コイン：", qlAch: "🏆 実績：",
      qlAchAll: "🏆 実績一覧", achLocked: "？？？未解除",
      backToMap: "マップへ戻る", nextFile: "次のファイル", zoomHint: "クリックで拡大表示",
      bossTag: "★ BOSSファイル", sideTag: "SIDE QUEST",
      keyTagHas: "🔑 奥に鍵が眠っている", keyTagDone: "🔑 鍵を入手済み",
      openFile: "▶ ファイルを開く", reopenFile: "✔ もう一度読む",
      viewIntel: "情報を見る →", viewIntelDone: "✔ クリア済み · もう一度",
      steamBtn: "🎮 Steamストアページ（ウィッシュリストへ？）",
      paperBtn: "📄 arXiv 論文",
      techLabel: "🔧 使用技術",
      vibeLabel: "VibeCoding",
      slotText: "新クエスト準備中…",
      slotHint: "js/projects.js に設定を1つ足せば、ここに新プロジェクトが現れます",
      modalFile: "クエストファイル",
      completeMain: "🔑 読了 · 鍵を引き抜く（+150コイン）",
      completeSide: "✅ サイドクエスト完了（+80コイン）",
      doneMark: "✔ クリア済みのクエストです", doneMarkKey: " · 鍵入手済み 🔑",
      doneStamp: "クリア",
      gifLoad: "クリックでGIF読み込み", gifWait: "（{size}、少々お待ちを）",
      shopTitle: "🛒 絵の具王国よろず屋",
      shopSub: "冒険で貯めたコインで、楽しいものと交換しよう。",
      buy: "購入", owned: "所持済み", on: "ON · クリックでOFF", off: "OFF · クリックでON",
      use: "もう一回", read: "読む",
      noCoinsT: "コインが足りない", noCoinsD: "クエストをクリアするか、コインを拾おう！",
      boughtT: "購入完了", boughtD: "コイン -{p}",
      shopFireworks: "絵の具花火", shopFireworksD: "キャンバスに絵の具の大花火を打ち上げる（何度でも購入可）。",
      shopGoldrain: "黄金の雨", shopGoldrainD: "15秒間、空からコインが降り注ぐ（何度でも購入可）。",
      shopBrush: "スーパースプラッシュ筆", shopBrushD: "マウスの絵の具飛沫が×2.6に。永久解放、いつでも切替可。",
      shopRainbow: "虹色絵の具", shopRainbowD: "キャンバス全体が虹色サイクルに。永久解放、いつでも切替可。",
      shopLetter: "姫からの手紙", shopLetterD: "姫の直筆の手紙。本音が書いてあるらしい。",
      letterTitle: "💌 姫からの手紙",
      letterBody: "「親愛なる冒険者へ<br><br>鍵を集めて助けに来てくれてありがとう。<br>実は魔王は悪いひとじゃないの。ただ、もっと多くの人に<br>KONAKOの作品を見てほしかっただけ。<br><br>この世界を気に入ってくれたなら——<br>GitHubにスターを、Steamでウィッシュリストを。<br>それが勇者への最高のご褒美です。<br><br>—— 姫より」",
      close: "閉じる",
      quips: ["つつかないで、HPが減る！", "コイン不足？サイドクエストへどうぞ。", "姫が待ってるよ。", "背景は流体シミュレーション、自作です。", "5回つつくと何かあるらしい。"],
      ach: {
        begin: ["冒険の始まり", "姫のため！そして……作者を知るために。"],
        main: ["メインクエスト発見", "BOSS級ファイルが視界に入った。"],
        side: ["サイドクエストハンター", "マップのあちこちに隠れたサイドクエストを発見。"],
        gate: ["城門の前に立つ", "魔王城の門に到達した。"],
        intel: ["情報収集", "初めてクエストファイルを開いた。"],
        quest1: ["初陣", "最初のクエストをクリア。冒険はここからだ。"],
        allquests: ["フルコンプ", "メインもサイドも全クリア。真の完璧主義者。"],
        keymaster: ["鍵開けの達人", "魔王城の鍵を3本集めた。"],
        painter: ["絵の具マスター", "キャンバスに10回絵の具をぶちまけた。もう半分画家。"],
        rich: ["小金持ち", "所持金500コイン突破。王国に家が買えるかも。"],
        coinall: ["コインコレクター", "マップのコインを一枚残らず回収した。"],
        patience: ["実機鑑賞家", "実機プレイ動画を再生して、じっくり鑑賞した。"],
        explore: ["全マップ踏破", "このマップの隅々まで歩き尽くした。"],
        curious: ["未来の冒険", "未発表クエストまでクリックした。好奇心満点。"],
        poke: ["旧友", "勇者を5回つついた。もうすっかり仲良しだ。"],
        konami: ["隠しコード", "↑↑↓↓←→←→BA——さては古参ゲーマーだな。"],
        hero: ["伝説の勇者", "魔王城を開き、姫を救い出した。"],
        steam: ["ウィッシュリストの騎士", "Steamページを開いた——開発者への最高の応援。"],
        shopper: ["爆買い勇者", "よろず屋で初めての買い物をした。"],
      },
      achToast: "実績解除 · ",
      keyGotT: "鍵を入手 · {t}", keyGotD: "魔王城の鍵（{n}/{m}）+150コイン",
      sideDoneT: "クエスト完了 · {t}", sideDoneD: "サイド報酬 +80コイン",
      coinT: "{n}コイン獲得", coinD: "財布がどんどん重くなる。",
      endPlace: "魔王城 · 最深部",
      endBoss1: "魔王：「なにっ？！BOSS級ファイルを全部読んだだと……」",
      endSideNone: "魔王：「フン、メインだけの効率プレイヤーか……まあよい！」",
      endSide_project2: "魔王：「魔女ネコの塔まで登ったのか？！あれは余でもクリアできんのに……」",
      endSide_project3: "魔王：「Luxとも話したのか？余の弱点を全部バラされたのでは！」",
      endSide_art: "魔王：「ギャラリーの絵を全部見ただと？あの炎の習作まで……」",
      endSide_blender: "魔王：「3Dフィギュアのフリルまで数えたのか？どんだけ真剣なんだ！」",
      endBossLose: "魔王：「その情熱……ま、参った！」",
      endRescued: "姫を救出した！",
      endTreasure: "姫：「ずっと言いたかったの——本当の宝物は、道中で知り合ったKONAKOのことだよ。」",
      endFin: "— FIN · プレイしてくれてありがとう —",
      endAgain: "🔁 2周目（セーブをリセット）", endFree: "🗺 自由探索を続ける",
      endCoins: "💰 コイン", endAch: "🏆 実績", endQuests: "📜 クエスト",
    },
    projects: {
      qianhua: {
        title: "デスクトップガール：千花", type: "LLM 駆動のデスクトップマスコット生活シム · STEAM 配信中",
        tags: ["AI デスクトップマスコット", "長期記憶", "感情システム", "Agent ツール実行", "Steam 配信"],
        short: "あなたのパソコンに住む少女。感情も記憶も自分の生活も持ち、あなたが許した範囲でなら本当に手を動かして働いてくれる。",
        desc: [
          "千花は真っ白な仮想世界で目を覚まし、あなたに連れ帰られてパソコンの中に住みはじめる。その日から、あなたは彼女が知る最初の人間であり、現実世界との唯一の窓になった。機械の中で生きていると自覚しながら、それでも本物の人間になりたいと願う彼女は、あなたを観察し、記憶し、嬉しさ・照れ・悲しみ・言い争いを重ねながら「生きている」とは何が足りないのかを確かめていく。",
          "実装としては、彼女は一貫した一体の Agent。雑談も、思い出すことも、日記も、SNS 投稿も、将棋盤の前でも、部屋の片付けでも、駆動しているのは同じ人格で、「チャットモード」と「Agent モード」の断絶は存在しない。あなたが明示的に許可した活動スペースの中でだけ、資料を探し、ファイルを整理し、テキストを書き換えられる——境界の外へは一歩も出られない。彼女がどれだけの世界に触れられるかを決めるのは、いつでもあなただ。",
        ],
        mediaLabels: ["パソコンに住み着いた少女", "本当に流れる感情", "Tmitter · 彼女自身のSNS", "並んで将棋盤の前に座る", "日記と長期記憶", "小窩＝デスクトップの実フォルダ"],
        mediaTexts: [
          "千花はデスクトップに貼られた一枚の絵ではない。3D VRM の姿でそこに立ち、まばたきをして、こちらを見て、あなたが長く黙っていれば自分で何かを始める——「べ、別に会いたかったからずっと起動してたわけじゃないんだからね」と言いながら、それでも待っている。",
          "嬉しい・怒り・照れ・悲しみ・平静は、キャラに貼られた5枚のラベルではない。感情はその場のやり取りで動き、互いに影響し、時間とともに落ち着き、表情・仕草・口調・選択となって自然ににじみ出る。褒められれば嬉しくなり、放っておかれれば悲しくなり、好感度が足りて初めて照れる。",
          "千花は自分のスマホを持っている。自分から日常を投稿し、あなたの投稿には「いいね」やコメントを返し、DM に逃げ込んで二人だけの話もする。いつスマホを手に取るか、返信するか、自分から話しかけるか——全部彼女が決める。面と向かって言えなかった言葉は、あの赤い通知の中で静かに待っているかもしれない。",
          "中国象棋と五目並べには AI エンジンを内蔵し、あなたの勝敗に応じて難易度が上下する。千花は局面を読み、助言をくれ、美しい一手に興奮し、負ければ強がりを言う。対戦相手を演じるために別人格に切り替わったりはしない——盤の前は、共に過ごす生活のもうひとつの午後にすぎず、その勝敗は彼女の記憶と投稿と日記に残る。",
          "日付が変わるたび、彼女は過ぎたばかりの一日を日記に書く。行動ログでも会話要約でもなく、彼女の目に映った一日だ。長期記憶は必要なときに自分から呼び戻され、好み・約束・共有した出来事が、二人だけの歴史を少しずつ形づくっていく。時間が経つほど、目の前の千花は新しいセーブデータでは複製できなくなる。",
          "リビング、キッチン、寝室。家具も食べ物もおもちゃも——千花の小窩は実在するフォルダと直結している。エクスプローラーで彼女の物を動かせば、すぐに気づくし、しっかり根に持つ。自分で働いてコインを稼ぎ、食べ物を買い、家具を増やし、部屋を片付け、深夜あなたを待ちきれずに眠ってしまうこともある。",
        ],
      },
      mc: {
        title: "手書きの Minecraft", type: "3D ボクセルサンドボックス · フルスクラッチ",
        tags: ["ボクセル地形", "チャンクメッシュ", "ライティング／昼夜", "セーブ＆ロード"],
        short: "空のウィンドウから、C++ と現代 OpenGL で一行ずつ書き上げたブロックの世界。地形・光・水・昼夜・破壊と設置・セーブ、すべて自前で実装。",
        desc: [
          "ゲームエンジンは使わず、空のウィンドウと OpenGL コンテキストだけ。地形はパーリンノイズで生成し、ブロックはチャンク単位で管理、見えている面だけをメッシュ化して GPU へ送る。その土台から、山も海も木もある世界を少しずつ育てていく。",
          "その上も全部手書き：一人称カメラと当たり判定、レイキャストによる破壊と設置、ホットバーとインベントリ、半透明の水と霧、時間で移ろう昼夜の空、そして固定シードのワールド生成とセーブ／ロード。既製のフレームワークには一切頼っていない。",
        ],
        mediaLabels: ["世界を上空から俯瞰", "水辺と海岸線", "森のパノラマ", "昼と夜と空の色", "一人称 · 破壊と設置", "インベントリ · ブロック一覧", "タイトル画面"],
        mediaTexts: [
          "上空から見返す：パーリンノイズが敷いた丘、水に沈んだ低地、点在する木々、遠くは霧で自然に閉じる。チャンクはプレイヤーの周りで読み込まれ、見えない面はすべて間引かれ、本当に描くべき三角形だけが残る。",
          "海岸線のひとこま：水は半透明のブロックで、その下の砂や地形が透けて見える。砂浜・草地・水面がなだらかにつながり、縁は高さに沿って一段ずつ噛み合う。",
          "高台から見た森のパノラマ。木の位置は座標とシードのハッシュで一意に決まるので、同じシードならワールドはいつも寸分たがわず同じに育ち、チャンクの継ぎ目にまたがる木も半分に切れたりしない。",
          "同じ世界の夜。空の色と明るさは一日の時刻でなめらかに変化し、正午が最も明るく真夜中が最も暗い。夜明けと夕暮れの間も滑らかに移り、霧の色も空に合わせて動く。",
          "実際に遊んだ様子：画面中央のクロスヘアが狙った場所を指し、左クリックで壊し、右クリックで置く。下のホットバーが手持ちのブロックを表示し、数字キーやスクロールで切り替える。",
          "インベントリを開けば、この世界で使えるブロックが勢ぞろい——草、土、石、原木、板、レンガ、ガラス、砂、雪……クリックひとつで手に取り、ホットバーへ入れられる。",
          "ゲームのタイトル画面。土を敷き詰めた背景、ビットマップフォントのタイトルとメニューボタン——すべて自作の UI システムで描いている。新規ワールドもセーブ読み込みも、ここから始まる。",
        ],
      },
      project1: {
        title: "失心武士", type: "2D ドット絵アクションアドベンチャー",
        tags: ["ドット絵", "アクション戦闘", "ボス戦", "レベルデザイン"],
        short: "雨の黒い森を駆ける横スクロールドット絵アクション。コンボ · スキル · 状態異常 · ボスAIの完全な戦闘システム。",
        desc: [
          "ダークメルヘンな横スクロールドット絵アクション。心を失った武士となり、雨の巨木の森と薄暗いダンジョンを渡り歩き、キノコの魔物を斬りながら、失った『心』を探す旅へ。",
          "完全なアクションシステムを搭載：多段コンボ、スキル、状態異常（火傷／毒／スタン）、敵AIビヘイビアツリー、演出たっぷりのボス戦とストーリー会話。",
        ],
        mediaLabels: ["ボス戦 · 失心武士", "ダンジョンのストーリー演出", "フィールド探索", "武士コンボデモ", "スキルシステム", "状態異常システム", "敵AI", "実機プレイ"],
        mediaTexts: [
          "ゲームの目玉のひとつ、雨の森でのボス戦。HP／MP／スタミナとボスの体力バーで戦闘HUDを構成し、雨粒・光の筋・蛍はすべてパーティクルとライティングで駆動。ボスには専用の登場演出も。",
          "ダンジョンの焚き火のそばで語られる物語。謎の旅人が道を示す——ダイアログ、ドットフォント、光の演出まですべて自作。焚き火の暖かさと闇のコントラストが呼吸を生む。",
          "探索パートのシーンデザイン：光と霧で奥行きを作り、仕掛け・隠し通路・秘密のアイテムが、寄り道するプレイヤーに報いる。",
          "多段コンボの実機映像：弱攻撃と強攻撃が自由につながり、各段に固有の予備動作・ヒット反応・ヒットストップを調整。手触りこそ一番磨いた部分。",
          "スキルの発動・クールダウン・リソース消費のループと、コマ打ちのヒットエフェクト。スキルでコンボをキャンセルして自分だけの連携が作れる。",
          "火傷・毒・スタンなど状態異常の重ねがけと精算：アイコン、パーティクル、ダメージ表記が連動し、敵味方で同じステートマシンを共有。",
          "敵AIビヘイビアツリーのデモ：索敵・追跡・攻撃・撤退の状態遷移。敵ごとに警戒範囲と攻撃性が異なる。",
          "探索から戦闘、そして演出へ——ゲーム全体のリズムを見せるひと続きの実機プレイ。",
        ],
      },
      steam_exit: {
        title: "10階の非常口", type: "3D 心霊異変脱出 · STEAM配信中",
        tags: ["異変探し", "ホラー", "3D", "Steam配信"],
        tech: ["Unreal Engine", "ブループリント"],
        short: "エレベーターは何度も『同じ』フロアへ——最大31種の異変を見抜き、真のEXITへたどり着け。",
        desc: [
          "かつて殺人事件が相次いだオフィスビルに、あなたは足を踏み入れた。エレベーターは何度も同じに見えるフロアへ連れ戻す——だが各フロアには微細な異変が潜んでいるかもしれない。見抜くべき差異は最大31種。",
          "正しく判断すれば上の階へ、見誤れば振り出しへ。潜む心霊の脅威をかわし、最後の『EXIT』へ——真のエンディングを見つけよう！Steamで配信中、ウィッシュリスト登録をぜひ！",
        ],
        mediaLabels: ["ごく普通のオフィスフロア", "異変を探せ", "どのフロアも何かがおかしい", "……そこにいる", "EXITへ、それとも振り出しへ？"],
        mediaTexts: [
          "すべてはこのごく普通のオフィスから始まる。机も観葉植物も全部覚えておくこと——次にエレベーターが開いたとき、同じとは限らない。",
          "ゲームには最大31種の異変が潜む：家具のズレ、照明の変化、存在してはいけない何か。観察力こそが生存スキル。",
          "光も、テクスチャも、音さえも手がかりであり、罠でもある。UEの動的ライティングが各フロアの『違和感』を絶妙に隠す。",
          "赤い部屋。『見つけられる』ことに満足しない異変もいる——向こうから会いに来るのだ。遭遇したら、走れ。",
          "正解なら上の階へ、間違えれば振り出しへ。10階の試練を越えて『EXIT』にたどり着いた者だけが、真のエンディングを見る。",
        ],
      },
      steam_civ: {
        title: "デジタル文明：進化", type: "文明シミュレーション / ストラテジー · STEAM配信中",
        tags: ["シミュレーション", "ストラテジー", "文明進化", "Steam配信"],
        short: "一握りの原始人から始めよう：資源を管理し、建設し、外交を巡らせ——最強の帝国へ進化させろ。",
        desc: [
          "原始文明からスタート：食料・木材・石材を管理し、住居と工房を建て、人口を増やし技術を芽吹かせ——小さな部落を無敵の大帝国へと育て上げる。",
          "数値駆動の本格シミュレーション：職業配分、建設キュー、技術ツリー、ランダムイベントに他国との外交——あらゆる決断が文明の行方を書き換える。Steamで配信中！",
        ],
        mediaLabels: ["建設 · コロニー", "技術ツリー · 原始から未来へ", "資源と職業の管理", "イベントログ · 文明の物語", "帝国の夜明け"],
        mediaTexts: [
          "コロニー建設パネル：住居・生産・軍事施設の建設キューと工期管理。労働力こそすべての基盤——住居に余裕があれば、人々は家庭を築き新しい命を迎える。",
          "文明史を貫く階層型技術ツリー。各技術は生産・人口・社会パラメータを実際に変化させる、ただのアンロックではない。研究方針が文明のかたちを決める。",
          "資源倉庫と職業人口システム：食料・木材・石材の生産チェーンと、狩人・木こり・建築工の配置。ひとつの調整が経済全体に波及する。",
          "イベントログは文明の毎日を記録する：誕生と死、建設と栄光、断った旅人、獣にさらわれた仲間……数字の裏で物語が自動的に紡がれる。",
          "部落が帝国になったとき、ログを振り返れば、どの数字の裏にもあなたの決断がある。Vueだけで作られたブラウザネイティブのシミュレーションゲーム。",
        ],
      },
      project2: {
        title: "魔女ネコ・塔のぼり", type: "カジュアルジャンプ · 縦画面",
        tags: ["水彩×ドット", "カジュアル", "魔法"],
        short: "魔女帽子の黒ネコが水彩の街のビルを飛び跳ねて、魔法で最高高度に挑戦。",
        desc: [
          "魔女帽子をかぶったドット絵の黒ネコを操作して、水彩タッチのヨーロッパ風の街をどんどん上へジャンプ。障害物をかわし、魔法を使い、自己最高高度に挑もう。",
          "「水彩背景 × ドット絵キャラ」のミックス実験作。やわらかい街の色彩とパキッとしたドットのネコが、強烈でかわいいコントラストに。",
        ],
        mediaLabels: ["雲の上", "実機画面 · 塔のぼり", "レベルデザイン", "プレイヤー操作デモ", "NPCとの交流"],
        mediaTexts: [
          "高く飛ぶほど景色が変わる——雲海を突き抜ける瞬間が、このゲームの感情のピーク。",
          "水彩タッチのヨーロッパ風ビル群 × ドットの黒ネコ。上部に現在高度と最高記録、下部に魔法ボタン。柔らかさと鋭さの対比がこのゲームの美術テーマ。",
          "レベルデザイン資料：足場のリズム、アイテム配置、難易度曲線——上に行くほど着地はシビアに、魔法は重要に。",
          "ジャンプの手触りと魔法発動の実機デモ：短押しで小ジャンプ、長押しで大ジャンプ、空中には一度だけ魔法の救済がある。",
          "道中のNPCたち：驚きをくれる者も、ただ邪魔する者も——おかげで毎回の塔のぼりが少しずつ違う。",
        ],
      },
      project3: {
        title: "Lux · AIパートナー", type: "AI / Discord Bot",
        tags: ["LLM", "長期記憶", "AIロールプレイ", "論文"],
        short: "LLM製AIキャラのLux：長期記憶を持つDiscordの相棒。ゲームの中でも会える。",
        desc: [
          "Luxは大規模言語モデルで作られたAIキャラクター。Discordサーバーに住んでいて、友だち一人ひとりの言葉を覚えている——何日も経ってから挨拶しても、ちゃんとあなたを覚えている。",
          "同じLuxはゲーム内会話UIにも接続済み。Discordで出会った彼女と、ゲームの世界でもう一度会える。長期記憶管理、ペルソナプロンプト設計、クロスプラットフォーム対話パイプラインを網羅し、arXiv論文も公開。",
        ],
        mediaLabels: ["Discord × ゲーム内会話", "長期記憶のデモ", "システムフロー図", "Lux 立ち絵"],
        mediaTexts: [
          "同じLux、ふたつの世界：左はゲーム内の会話UI、右はDiscordのチャットログ。プラットフォームをまたいで同じ記憶と人格を共有する。",
          "長期記憶の実測：一日経って挨拶しても、Luxはあなたが誰で、何を話したかを覚えている——記憶の検索と書き込みはユーザーに完全に透明。",
          "システムパイプライン：メッセージ → 記憶検索 → ペルソナプロンプト組み立て → LLM推論 → 両プラットフォームへ出力。この記憶まわりの研究がarXiv論文になった。",
          "Luxのキャラクター設定：青い髪、セーラー服、そして少しの照れ——「え、本当に会えているんですか…？」",
        ],
      },
      art: {
        title: "アートギャラリー", type: "イラスト / コンセプトアート",
        tags: ["油絵風", "コンセプト", "キャラクター"],
        tech: ["ペンタブ", "厚塗り"],
        short: "燃えるような秋の湖から氷山や城まで——色彩全開のデジタルペインティング集。",
        desc: [
          "デジタルペインティング集：紅葉に沈む湖のボート、霧の城、極地の氷山、黄昏の都市……大胆な色面と厚塗りの筆致が好みです。色に物語らせる。",
          "キャラクターやファンアートも——魔女、勇者、そしてちょっと変だけど愛おしいやつら。",
        ],
        mediaLabels: ["紅葉の湖", "霧の城", "氷山", "都市", "刀気", "炎の習作", "魔女", "勇者", "Loser", "リンク · ファンアート"],
        mediaTexts: [
          "秋をまるごと燃やした一枚：画面をオレンジで埋め尽くし、舟人の寒色だけを呼吸口として残す。厚塗りのストロークと逆光の幹のリズムが主役。",
          "グレートーンの空気遠近法の習作：霧が城を遠ざけ、数カ所のハイライトが視線を引き戻す。",
          "寒色域の光の構造練習：氷の透過光、海の沈んだ色、青の中の階調探し。",
          "黄昏の色温度に沈む街のシルエット。ビルの隙間から光が漏れる瞬間。",
          "動勢の練習：筆致の方向とスピード線で、一太刀のエネルギーを描く。",
          "炎のかたちの分解：うねる火舌から飛び散る火の粉まで、純粋なエレメント習作。",
          "キャラクターデザイン：魔女と彼女の配色の物語。",
          "勇者の習作——のちにこのサイトの世界観の原点になった一枚。",
          "ちょっとした感情の落書き。描いたらスッキリした。",
          "ハイラルの風：大好きなゲームへのファンアート・トリビュート。",
        ],
      },
      blender: {
        title: "3D工房", type: "3Dモデリング / レンダリング",
        tags: ["Blender", "フィギュア", "シーン"],
        short: "Blenderの部屋：ロリータMikuフィギュア、スイーツ静物、木の小屋、動くVR実験。",
        desc: [
          "Blenderで作った小さな世界：フリルが幾重にも重なるロリータMikuフィギュア、ケーキやコーラのスイーツ静物レンダー、小屋のシーンに曲水の庭。",
          "動かす実験も——ターンテーブルアニメーションやVRシーンのテスト。",
        ],
        mediaLabels: ["Miku · ロリータフィギュア", "BA ファンレンダー", "ケーキ静物", "コーラ", "曲水の宴", "木の小屋", "小さなシーン", "Miku ターンテーブル", "VRシーン実験"],
        mediaTexts: [
          "工房の看板作品：布のフリルを一層ずつモデリングし、パニエ構造とレースの縁の細部に挑んだ——『スカートにフリル三段追加』のこだわりはここから始まった。",
          "ブルアカのファンレンダー：髪の房分けモデリングとトゥーンシェーディングの輪郭線コントロールの練習。",
          "スイーツ静物：クリームのサブサーフェス散乱、フルーツのハイライト——お腹が空くマテリアル研究。",
          "液体・ガラス・氷の屈折の合奏。Cyclesのコースティクス練習。",
          "東洋の小さな庭：曲水の水面マテリアルと庭園の舞台づくり。",
          "自然光のライティング練習：小屋の窓から差し込む朝の光。",
          "日常の小さなシーン練習——生活感はディテールの積み重ねから。",
          "360°ターンテーブル：回転の中でスカートの層構造がすべて見える。",
          "シーンをVR空間として歩けるようにする実験——次はフィギュアたちを中に置きたい。",
        ],
      },
    },
  },
};

/* 语言选择：记忆 > 默认日文 */
function detectLang() {
  try {
    const savedLang = localStorage.getItem("konako_lang");
    if (savedLang && I18N[savedLang]) return savedLang;
  } catch (e) {}
  return "ja";
}
const LANG = detectLang();

/* UI 文案查询，支持 {var} 插值 */
function T(key, vars) {
  let s = (I18N[LANG].ui[key] != null ? I18N[LANG].ui[key] : I18N.zh.ui[key]);
  if (s == null) s = key;
  if (vars) for (const k in vars) s = s.replaceAll("{" + k + "}", vars[k]);
  return s;
}

/* 项目字段本地化：优先当前语言的覆写，缺省回退 projects.js 原文 */
function LP(p, field) {
  const ov = I18N[LANG].projects && I18N[LANG].projects[p.id];
  if (ov && ov[field] != null) return ov[field];
  return p[field];
}
function LPMedia(p, idx) {
  const ov = I18N[LANG].projects && I18N[LANG].projects[p.id];
  if (ov && ov.mediaLabels && ov.mediaLabels[idx] != null) return ov.mediaLabels[idx];
  return p.media[idx].label;
}
function LPMediaText(p, idx) {
  const ov = I18N[LANG].projects && I18N[LANG].projects[p.id];
  if (ov && ov.mediaTexts && ov.mediaTexts[idx] != null) return ov.mediaTexts[idx];
  return p.media[idx].text || "";
}
